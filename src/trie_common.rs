...
pub trait ContainsTrieNode<'a, K: 'a, V: 'a> where K: TrieKey {
    fn trie_node(self) -> &'a TrieNode<K, V>;
}

/// Regular trie.
impl <'a, K: 'a, V: 'a> ContainsTrieNode<'a, K, V> for &'a Trie<K, V> where K: TrieKey {
    fn trie_node(self) -> &'a TrieNode<K, V> {
        &self.node
    }
}

impl <'a, K: 'a, V: 'a> TrieCommon<'a, K, V> for &'a Trie<K, V> where K: TrieKey {
    fn len(self) -> usize {
        self.length
    }
...
/// Subtrie.
impl <'a: 'b, 'b, K: 'a, V: 'a> ContainsTrieNode<'a, K, V> for &'b SubTrie<'a, K, V> where K: TrieKey {
    fn trie_node(self) -> &'a TrieNode<K, V> {
        self.node
    }
}
